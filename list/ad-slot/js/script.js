function init_menu()
{
	...
	$(".menu_ac").find("[href='/"+page+"']").addClass("active");
	$(".submenu_right").find("[href='/"+page+"']").addClass("active");

	if (page == 'adv-banners') {$(".submenu_right").find("[href='/adv-links']").addClass("active");}
	if (page == 'web-banners') {$(".submenu_right").find("[href='/web-links']").addClass("active");}
...
$(document).ready(function()
{
	...
	$('.btn_sh').click(function(event)
	{
		...
		{
			if ($('#sel_bz1').prop('checked'))
				$('#construct_btn').click();
			if ($('#sel_bz2').prop('checked'))
				$('#code_btn').click();
	...
	$('.change_event').change(function(event) {
		...
		$('.dummy_block .head_d').text(head_p_text).css({
			'color': head_p_color1,
		...
		{
			$('.dummy_block').css({
				'background': 'repeating-linear-gradient(-45deg, transparent, transparent 20px, rgba(255, 255, 255, 0.13) 20px, rgba(255, 255, 255, 0.13) 40px)',
				'background-color': bg_color1,
				'height': height_banner1 + 'px',
				'width': width_banner1 + 'px',
				'border': border_size1 + 'px solid' + border_color1,
		...
		{
			$('.dummy_block').css({
				'background': 'repeating-radial-gradient(circle at center center, rgba(0, 0, 0, 0.2), rgba(0, 0, 0, 0.2) 2px, transparent 2px, transparent 100%) 0% 0%',
				'background-color': bg_color1,
		...
		{
			$('.dummy_block').css({
				'background': 'repeating-radial-gradient(circle at center, rgba(0,0,0,.2), rgba(0,0,0,.2) 1px, transparent 1px, transparent 100%)',
				'background-color': bg_color1,
