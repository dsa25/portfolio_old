...
class db
{
	...
	private $defaults = array(
		...
		'exception' => 'Null', //Exception class name
	);
	...
	private function prepareQuery($args)
	{
		...
		$array = preg_split('~(\?[nsiuapy])~u',$raw,null,PREG_SPLIT_DELIM_CAPTURE);
		$anum  = count($args);
	...
	private function createSET2($data)
	{
		if (!is_array($data))
		{
			$this->error("SET (?y) placeholder expects array, ".gettype($data)." given");
			return;
		}
		if (!$data)
		{
			$this->error("Empty array for SET (?y) placeholder");
			return;
		}
		$query = $comma = '';
		foreach ($data as $key => $value)
		{
			if ($value[0]=="+")
			{
				...
				$query .= $comma.$this->escapeIdent($key).'='.$this->escapeIdent($key)." + ".$this->escapeString($value);
			}
			else
			if ($value[0]=="-")
			{
	...
	public function log_file($value='')
	{
		$fp = fopen($_SERVER['DOCUMENT_ROOT']."/log_db.txt", "a");
